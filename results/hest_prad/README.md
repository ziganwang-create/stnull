# PRAD -- zero-pixel audit results

HEST-Benchmark cohort `PRAD`, audited with `stnull`. Everything here is a
...
| sd(log lib) range | 0.466-3.407 |
| Sparsity (zero fraction) | 0.9406 |
| Zero-pixel null, official caliber | 0.5401 |
| Best published model | GenBio-PathFM |
| Its published r | 0.3913 |
| Models the null matches or beats | 26/26 (also 26/26 under cp10k_panel) |
| Null as pct of best | 138% |
| Ceiling r_tech (median) | 0.687 med (0.41-0.94) |
| r_nbr (median) | 0.452 med (0.18-0.84) |
| H2 zero-biology sim >= real | 66/92 |
| H3 ordering verdict | REPLICATES (Spearman 0.79 >= prereg 0.5) |

Row source: `../hb_MASTER_TABLE.csv`, cohort `PRAD`.
...
| file | what it holds |
|---|---|
| `hb_prad_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_prad_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_prad_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_prad_libnull_folds.csv` | H1 null per official fold, behind the `_official` mean |
| `hb_prad_libnull_official.csv` | H1 null under the exact official convention: the number comparable to a leaderboard cell |
| `hb_prad_panels.json` | exact gene lists used, per fold and spot set |
| `hb_prad_prereg.json` | pre-registration written before the audit ran |
| `hb_prad_struct.json` | split structure and per-sample patient mapping behind the leakage verdict |
| `hb_prad_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_prad_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_prad_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |

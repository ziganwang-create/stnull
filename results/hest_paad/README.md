# PAAD -- zero-pixel audit results

HEST-Benchmark cohort `PAAD`, audited with `stnull`. Everything here is a
...
| sd(log lib) range | 2.101-4.106 |
| Sparsity (zero fraction) | 0.5638 |
| Zero-pixel null, official caliber | 0.5250 |
| Best published model | Virchow |
| Its published r | 0.5159 |
| Models the null matches or beats | 26/26 |
| Null as pct of best | 102% |
| Ceiling r_tech (median) | ~0.93 (0.912-0.942) |
| r_nbr (median) | 0.62-0.83 |
| H2 zero-biology sim >= real | 12/12 |
| H3 ordering verdict | REPLICATES 4/4 cells (but P3 raw>cp10k FALSIFIED) |

Row source: `../hb_MASTER_TABLE.csv`, cohort `PAAD`.
...
| file | what it holds |
|---|---|
| `hb_paad_audit_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_paad_audit_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_paad_audit_main.py` | the audit driver that produced this cohort's CSVs |
| `hb_paad_audit_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_paad_audit_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_paad_audit_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |
| `hb_paad_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_paad_inspect.py` | first-pass inspection of the raw cohort files |
| `hb_paad_leaderboard_compare.csv` | published models for this cohort placed next to the null |
| `hb_paad_panels.json` | exact gene lists used, per fold and spot set |
| `hb_paad_prereg.md` | pre-registration written before the audit ran |
| `hb_paad_prereg_stamp.txt` | pre-registration written before the audit ran |
| `hb_paad_struct.json` | split structure and per-sample patient mapping behind the leakage verdict |

# LUNG -- zero-pixel audit results

HEST-Benchmark cohort `LUNG`, audited with `stnull`. Everything here is a
...
| sd(log lib) range | 2.402-2.464 |
| Sparsity (zero fraction) | 0.4889 |
| Zero-pixel null, official caliber | 0.6942 |
| Best published model | GenBio-PathFM |
| Its published r | 0.5787 |
| Models the null matches or beats | 26/26 |
| Null as pct of best | 120% |
| Ceiling r_tech (median) | ~0.96 (0.954/0.964) |
| r_nbr (median) | 0.830/0.821 |
| H2 zero-biology sim >= real | 8/8 |
| H3 ordering verdict | PARTIAL (order 4/4 but n=2; saturation point-prediction falsified) |

Row source: `../hb_MASTER_TABLE.csv`, cohort `LUNG`.
...
| file | what it holds |
|---|---|
| `hb_lung_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_lung_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_lung_leaderboard_compare.csv` | published models for this cohort placed next to the null |
| `hb_lung_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_lung_prereg.md` | pre-registration written before the audit ran |
| `hb_lung_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_lung_structure_order.csv` | H3 predicted vs observed sample ordering |
| `hb_lung_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |

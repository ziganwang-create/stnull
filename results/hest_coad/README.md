# COAD -- zero-pixel audit results

HEST-Benchmark cohort `COAD`, audited with `stnull`. Everything here is a
...
| Platform | Xenium |
| Samples | 4 |
| Patients | 2 groups (1 preview donor unverifiable + 3 CRC patients mislabeled as one in HEST meta) |
| Official split | official 2-fold |
| Leakage verdict | no provable leak (conservative under both mappings); meta mislabel documented |
| sd(log lib) range | 0.610-0.964 (official) / 1.576-1.697 (all) |
| Sparsity (zero fraction) | 0.3967 |
| Zero-pixel null, official caliber | 0.3218 |
| Best published model | GenBio-PathFM |
| Its published r | 0.3284 |
| Models the null matches or beats | 25/26 (rank 2; gap 0.0066) |
| Null as pct of best | 98% |
| Ceiling r_tech (median) | ~0.85 (0.75-0.95) |
| r_nbr (median) | 0.39-0.79 |
| H2 zero-biology sim >= real | 32/32 |
| H3 ordering verdict | PARTIAL 5/8 cells (official spot set falsified) |

Row source: `../hb_MASTER_TABLE.csv`, cohort `COAD`.
...
| file | what it holds |
|---|---|
| `hb_coad_audit_main.py` | the audit driver that produced this cohort's CSVs |
| `hb_coad_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_coad_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_coad_inspect.py` | first-pass inspection of the raw cohort files |
| `hb_coad_leaderboard_compare.csv` | published models for this cohort placed next to the null |
| `hb_coad_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_coad_libnull_folds.csv` | H1 null per official fold, behind the `_official` mean |
| `hb_coad_libnull_official.csv` | H1 null under the exact official convention: the number comparable to a leaderboard cell |
| `hb_coad_panels.json` | exact gene lists used, per fold and spot set |
| `hb_coad_prereg.json` | pre-registration written before the audit ran |
| `hb_coad_struct.json` | split structure and per-sample patient mapping behind the leakage verdict |
| `hb_coad_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_coad_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_coad_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |

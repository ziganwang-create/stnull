# LYMPH_IDC -- zero-pixel audit results

HEST-Benchmark cohort `LYMPH_IDC`, audited with `stnull`. Everything here is a
...
| sd(log lib) range | 1.169-2.265 (all_spots) |
| Sparsity (zero fraction) | 0.8375 |
| Zero-pixel null, official caliber | 0.3263 |
| Best published model | GenBio-PathFM |
| Its published r | 0.2842 |
| Models the null matches or beats | 26/26 |
| Null as pct of best | 115% |
| Ceiling r_tech (median) | 0.539 (official cal; 737/cp10k 0.522) |
| r_nbr (median) | 0.4244 (official mean-r) |
| H2 zero-biology sim >= real | 28/32 |
| H3 ordering verdict | REPLICATES 4/4 prereg cells |

Row source: `../hb_MASTER_TABLE.csv`, cohort `LYMPH_IDC`.
...
| file | what it holds |
|---|---|
| `lymph_AUDIT.md` | written narrative of this cohort's audit |
| `lymph_audit_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `lymph_audit_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `lymph_audit_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `lymph_audit_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `lymph_audit_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |
| `lymph_caliber_moran.csv` | Moran's I of the target under each caliber, plus dead-gene counts |
| `lymph_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `lymph_inspect.md` | first-pass inspection of the raw cohort files |
| `lymph_panels.json` | exact gene lists used, per fold and spot set |
| `lymph_stnull_dogfood.csv` | the same sections re-run through the shipped `stnull` package as a cross-check |
| `lymph_stnull_dogfood_notes.txt` | run notes for that cross-check |

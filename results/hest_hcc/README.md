# HCC -- zero-pixel audit results

HEST-Benchmark cohort `HCC`, audited with `stnull`. Everything here is a
...
| sd(log lib) range | 0.627-0.683 |
| Sparsity (zero fraction) | 0.8554 |
| Zero-pixel null, official caliber | 0.1484 |
| Best published model | STFlow (paper Table 1; no README column) |
| Its published r | 0.124 |
| Models the null matches or beats | 9/9 |
| Null as pct of best | 120% |
| Ceiling r_tech (median) | ~0.65 (0.629/0.679) |
| r_nbr (median) | 0.317/0.381 |
| H2 zero-biology sim >= real | 16/16 |
| H3 ordering verdict | PARTIAL 4/6 cells (one cp10k reversal beyond band) |

Row source: `../hb_MASTER_TABLE.csv`, cohort `HCC`.
...
| file | what it holds |
|---|---|
| `hb_hcc_audit_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_hcc_audit_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_hcc_audit_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_hcc_audit_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_hcc_audit_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |
| `hb_hcc_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_hcc_leaderboard.csv` | published models for this cohort placed next to the null |
| `hb_hcc_prereg.txt` | pre-registration written before the audit ran |

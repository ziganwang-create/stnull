# READ -- zero-pixel audit results

HEST-Benchmark cohort `READ`, audited with `stnull`. Everything here is a
...
| Platform | Visium |
| Samples | 4 |
| Patients | 2 (2 replicates/patient) |
| Official split | official 2-fold patient-level |
| Leakage verdict | no leak |
| sd(log lib) range | 0.550-0.878 |
| Sparsity (zero fraction) | 0.8892 |
| Zero-pixel null, official caliber | 0.2451 |
| Best published model | H-Optimus-1 |
| Its published r | 0.2421 |
| Models the null matches or beats | 26/26 |
| Null as pct of best | 101% |
| Ceiling r_tech (median) | ~0.70 (0.630-0.779) |
| r_nbr (median) | 0.355-0.547 |
| H2 zero-biology sim >= real | 15/16 |
| H3 ordering verdict | REPLICATES 4/4 cells + 4/4 point-in-band |

Row source: `../hb_MASTER_TABLE.csv`, cohort `READ`.
...
| file | what it holds |
|---|---|
| `hb_read_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_read_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_read_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_read_libnull_folds.csv` | H1 null per official fold, behind the `_official` mean |
| `hb_read_libnull_official.csv` | H1 null under the exact official convention: the number comparable to a leaderboard cell |
| `hb_read_panels.json` | exact gene lists used, per fold and spot set |
| `hb_read_prereg.json` | pre-registration written before the audit ran |
| `hb_read_struct.json` | split structure and per-sample patient mapping behind the leakage verdict |
| `hb_read_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_read_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_read_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |

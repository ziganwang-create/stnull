# IDC -- zero-pixel audit results

HEST-Benchmark cohort `IDC`, audited with `stnull`. Everything here is a
...
|---|---|
| Organ | breast |
| Platform | Xenium (pseudo-Visium binned) |
| Samples | 4 |
| Patients | 4 |
| Official split | official LOO=patient-level |
| Leakage verdict | no split leak; var50 panel-selection leak inherited (49/50 pooled repro) |
| sd(log lib) range | 0.828-1.377 (bench) / 1.066-3.907 (all) |
| Sparsity (zero fraction) | 0.6223 |
| Zero-pixel null, official caliber | 0.4284 |
| Best published model | H-Optimus-1 |
| Its published r | 0.6024 |
| Models the null matches or beats | 0/26 |
| Null as pct of best | 71% |
| Ceiling r_tech (median) | ~0.95 (0.919-0.979) |
| r_nbr (median) | 0.661-0.782 (bench log1p) |
| H2 zero-biology sim >= real | 32/32 |
| H3 ordering verdict | PARTIAL 6/8 cells |

Row source: `../hb_MASTER_TABLE.csv`, cohort `IDC`.
...
| file | what it holds |
|---|---|
| `hb_idc_audit_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_idc_audit_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_idc_audit_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_idc_audit_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_idc_audit_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |
| `hb_idc_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_idc_panels.json` | exact gene lists used, per fold and spot set |
| `hb_idc_prereg.md` | pre-registration written before the audit ran |

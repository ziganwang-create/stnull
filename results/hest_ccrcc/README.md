# CCRCC -- zero-pixel audit results

HEST-Benchmark cohort `CCRCC`, audited with `stnull`. Everything here is a
...
| Samples | 24 |
| Patients | 24 |
| Official split | official 6-fold x4 = patient-level |
| Leakage verdict | no leak; var50 pooled-selection leak reproduced 50/50; FFPE/frozen batch mix disclosed |
| sd(log lib) range | 0.382-1.394 (23/24 inside HER2ST fit domain) |
| Sparsity (zero fraction) | 0.8223 |
| Zero-pixel null, official caliber | 0.4945 |
| Best published model | Virchow2 |
| Its published r | 0.2719 |
| Models the null matches or beats | 26/26 |
| Null as pct of best | 182% |
| Ceiling r_tech (median) | 0.569 med (0.37-0.91; cp10k_panel 0.459; stnull-crosschecked max diff 0.02) |
| r_nbr (median) | 0.343 med (0.16-0.75; cp10k_panel 0.207) |
| H2 zero-biology sim >= real | 85/96 |
| H3 ordering verdict | REPLICATES (Spearman 0.799 >= prereg 0.5; prereg band [0.18-0.42] exceeded on conservative side) |

Row source: `../hb_MASTER_TABLE.csv`, cohort `CCRCC`.
...
| file | what it holds |
|---|---|
| `hb_ccrcc_audit_ceiling.csv` | H4 technical ceiling r_tech by split-half thinning, plus r_nbr where present |
| `hb_ccrcc_audit_libnull.csv` | H1 zero-pixel library-size null, per sample x panel x spot set x caliber |
| `hb_ccrcc_audit_libnull_foldconcat.csv` | H1 null with the fold's test samples concatenated before correlating, which is HEST's own aggregation |
| `hb_ccrcc_audit_rnbr.csv` | H5 neighbour-mean correlation r_nbr: the floor that spatial smoothness alone buys |
| `hb_ccrcc_audit_structure.csv` | H3 per-sample sd(log lib), the HER2ST-fitted prediction, and the residual |
| `hb_ccrcc_audit_structure_corr.csv` | H3 correlation of sd(log lib) with the null's score across samples |
| `hb_ccrcc_audit_zerobio.csv` | H2 zero-biology simulation: the same null on counts with all spatial signal removed |
| `hb_ccrcc_depth_pretable.csv` | pre-registered descriptive statistics of sequencing depth |
| `hb_ccrcc_leaderboard_compare.csv` | published models for this cohort placed next to the null |
| `hb_ccrcc_panels.json` | exact gene lists used, per fold and spot set |
| `hb_ccrcc_prereg.md` | pre-registration written before the audit ran |
